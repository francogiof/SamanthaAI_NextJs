...
export default function SelectRoleClient({ stackAuthId }: { stackAuthId: string }) {
  ...
  useEffect(() => {
    if (stackAuthId) {
      // Set stackAuthId cookie for server-side routing
      document.cookie = `stackAuthId=${stackAuthId}; path=/; max-age=2592000`; // 30 days
    }
  }, [stackAuthId]);
