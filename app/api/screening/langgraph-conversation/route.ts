...
export async function POST(req: NextRequest) {
  try {
    ...
    // --- Save candidate answer for progress tracking ---
    // Always save the answer for the step that was just answered (currentStep before increment)
    const answeredStepIndex = currentStep; // The step just answered
    const answeredStepData = interviewSteps[answeredStepIndex] as any;
    if (answeredStepData && candidateMessage) {
      try {
        db.prepare(`
          INSERT OR REPLACE INTO candidate_answers_table 
          (candidate_id, question_id, question_type, user_answer, score, feedback)
          VALUES (?, ?, ?, ?, ?, ?)
        `).run(
          candidateId,
          answeredStepData.step_id,
          answeredStepData.type,
          candidateMessage,
          null,
          null
        );
      } catch (err) {
        console.error('[LangGraph] Error saving candidate answer:', err);
      }
    }

    // --- Build allStepsWithStatus for progress bar ---
    let allAnswers: any[] = [];
    try {
      // Always order steps by step_order for correct progress and question order
      interviewSteps.sort((a: any, b: any) => a.step_order - b.step_order);
      const stepIds = interviewSteps.map((s: any) => s.step_id).join(',');
      allAnswers = db.prepare(`
        SELECT question_id, user_answer FROM candidate_answers_table
        WHERE candidate_id = ? AND question_id IN (${stepIds})
      `).all(candidateId);
    } catch (err) {
      console.error('[LangGraph] Error loading candidate answers:', err);
    }
    const allStepsWithStatus = interviewSteps.map((step: any) => ({
      step_id: step.step_id,
      structure: step.structure || '', // Always include structure for progress bar
      step_order: step.step_order,
      status: allAnswers.find((a: any) => a.question_id === step.step_id) ? 'completed' : 'pending'
    }));
    ...
    return NextResponse.json({ 
      ...
      currentStep: nextStep, // Always return the next step index for the frontend
      totalSteps: interviewSteps.length,
      interviewComplete,
      memory: updatedMemory,
      shouldMove: agentResult.shouldMove,
      reason: agentResult.reason,
      allStepsWithStatus // <-- for progress bar
    });
